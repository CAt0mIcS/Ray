...
#define P(x, y, w, arr) \
	(vec3{ arr[(3 * (((y)*w) + x))], arr[(3 * (((y)*w) + x)) + 1], arr[(3 * (((y)*w) + x)) + 2] })
...
{
	edge_color_t combined = edge_color_t(*color & banned);
...
void cubic_split(edge_segment_t* e, edge_segment_t* p1, edge_segment_t* p2, edge_segment_t* p3)
{
	...
	if (e->p[2][0] == e->p[3][0] && e->p[2][1] == e->p[3][1])
	{
...
float* ex_msdf_glyph(
	stbtt_fontinfo* font, uint32_t c, size_t w, size_t h, ex_metrics_t* metrics, int autofit)
{
	float* bitmap = (float*)malloc(sizeof(float) * 3 * w * h);
	...
	indices_t* contours = (indices_t*)malloc(sizeof(indices_t) * contour_count);
	int j = 0;
	for (int i = 0; i <= num_verts; i++)
	{
		if (verts[i].type == STBTT_vmove)
	...
	contour_t* contour_data = (contour_t*)malloc(sizeof(contour_t) * contour_count);
	float cscale = 64.0f;
	for (int i = 0; i < contour_count; i++)
	{
		size_t count = contours[i].end - contours[i].start;
		contour_data[i].edges = (edge_segment_t*)malloc(sizeof(edge_segment_t) * count);
	...
	int* corners = (int*)malloc(sizeof(int) * corner_count);
	int corner_index = 0;
	for (int i = 0; i < contour_count; ++i)
	{
		...
		else if (corner_index == 1)
		{
			...
			else if (contour_data[i].edge_count >= 1)
			{
				...
				edge_split(&contour_data[i].edges[0], parts[0 + 3 * corner], parts[1 + 3 * corner],
					parts[2 + 3 * corner]);
				if (contour_data[i].edge_count >= 2)
				{
					edge_split(&contour_data[i].edges[1], parts[3 - 3 * corner],
				...
				contour_data[i].edges = (edge_segment_t*)malloc(sizeof(edge_segment_t) * 7);
				contour_data[i].edge_count = 0;
				int index = 0;
	...
	for (int i = 0; i < contour_count; i++)
	{
		if (contour_data[i].edge_count == 1)
		{
			...
			free(contour_data[i].edges);
			contour_data[i].edges = (edge_segment_t*)malloc(sizeof(edge_segment_t) * 3);
	...
	// calculate windings
	int* windings = (int*)malloc(sizeof(int) * contour_count);
	for (int i = 0; i < contour_count; i++)
	{
	...
	contour_sd = (multi_distance_t*)malloc(sizeof(multi_distance_t) * contour_count);
	...
	for (int y = 0; y < h; ++y)
	{
		...
		for (int x = 0; x < w; ++x)
		{
			vec2 p = { (x + xoff - translate_x) / (scale * 64.0f),
				(y + yoff - translate_y) / (scale * 64.0f) };
			...
			for (int j = 0; j < contour_count; ++j)
			{
				...
				double med_min_dist =
					fabs(median(r.min_distance.dist, g.min_distance.dist, b.min_distance.dist));
				...
				med_min_dist =
					median(r.min_distance.dist, g.min_distance.dist, b.min_distance.dist);
			...
			if (pos_dist >= 0 && fabs(pos_dist) <= fabs(neg_dist))
			{
				...
				winding = 1;
				for (int i = 0; i < contour_count; ++i)
					if (windings[i] > 0 && contour_sd[i].med > msd.med &&
	...
	clashes_t* clashes = (clashes_t*)malloc(sizeof(clashes_t) * w * h);
	size_t cindex = 0;

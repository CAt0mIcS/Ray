...
namespace At0::Ray
{
	IndexBuffer::IndexBuffer(const RenderContext& context, CommandPool& transientCommandPool,
		std::string_view tag, const std::vector<Type>& indices)
		: Buffer(context, sizeof(IndexBuffer::Type) * indices.size(),
	...
	std::string IndexBuffer::GetUID(const RenderContext& context, CommandPool& transientCommandPool,

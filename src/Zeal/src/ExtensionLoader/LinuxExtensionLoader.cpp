...
namespace At0::Zeal
{
	void ExtensionLoader::Start(std::string_view searchPath, const std::function<void(Ray::Layer*)>& onNewLayer)
	{
		...
		Ray::Log::Debug("[ExtLoader] Startup");
		...
		{
			Ray::Log::Debug("[ExtLoader] Checking file {0}", dir.path().string());
			...
			if (!layerCreateFunc)
			{
				Ray::Log::Warn("[ExtLoader] Was unable to find 'CreateLayer' function in Module '{0}' with path '{1}'", handle, std::filesystem::absolute(dir.path().string()));
				dlclose(handle);
				continue;
			}

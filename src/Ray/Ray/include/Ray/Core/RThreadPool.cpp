...
namespace At0::Ray
{
	...
	void ThreadPool::Shutdown()
	{
		...
		Log::Debug("[ThreadPool] Joining Threads");
		for (uint16_t i = 0; i < m_MaxThreads; ++i)
		{
			auto id = m_Threads[i].get_id();
			Log::Debug("[ThreadPool] Joining Thread {0}", id);
			m_Threads[i].join();
			Log::Debug("[ThreadPool] Thread {0} joined", id);
		}
		Log::Debug("[ThreadPool] Finnished joining Threads");
	}
	...
	void ThreadPool::InfiniteWait()
	{
		...
		while (!m_Shutdown)
		{
			...
			Log::Debug("[ThreadPool] Thread {0} entered ThreadPool::InfiniteWait", std::this_thread::get_id());
			...
			if (task)
			{
				Log::Debug("[ThreadPool] Thread {0} Task {1} Execution started", std::this_thread::get_id(), &task);
				task();
				Log::Debug("[ThreadPool] Thread {0} Task {1} Execution finnished", std::this_thread::get_id(), &task);
			}
		}
	}
}

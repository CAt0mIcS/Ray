...
namespace At0::Ray
{
	Renderer3D::Renderer3D(EventDispatcher<WindowResizeEvent>& dispatcher)
		: m_Projection(MatrixIdentity()), m_pSwapChain(nullptr), m_pTarget(nullptr), m_hWnd(0),
	...
	{
		// RAY_TODO: Fix memory leak when resizing! (PRIORITY, MEMORY ALLOCATIONS UP TO 2GB)
		...
		SetProjection(MatrixPerspectiveLH(1.0f, size.y / size.x, 0.5f, 500.0f));
	}
}
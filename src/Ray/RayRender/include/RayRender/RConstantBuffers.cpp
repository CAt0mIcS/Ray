...
{
	template<typename T>
	Scope<ConstantBuffer> ConstantBuffer::Create(const T& data)
	{
		...
		{
#ifdef _WIN32
		case RendererAPI::D3D11:	return MakeScope<DX11ConstantBuffer<T>>(data);
#endif
		case RendererAPI::OpenGL:	return MakeScope<OpenGLConstantBuffer<T>>(data);
		}

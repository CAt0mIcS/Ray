...
#ifdef _WIN32
#	define RL_ASSERT(condition, msg, ...)	if(!(condition)) _wassert(::At0::Ray::Debug::RlAssert::AssertW(msg, __VA_ARGS__).c_str(), RL_WIDE(__FILE__), (unsigned int)__LINE__)
#elif defined(__linux__)
	#define RL_ASSERT(condition, msg, ...)	if(!(condition)) __assert(::At0::Ray::Debug::RlAssert::AssertA(msg, ## __VA_ARGS__).c_str(), __FILE__, (unsigned int)__LINE__)
#endif
